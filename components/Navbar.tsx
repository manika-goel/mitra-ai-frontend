...
/* ✅ Translation-safe nav keys */
const navLinks = [
  { key: "navAiTherapy", href: "/ai-therapy" },
  { key: "navAbout", href: "/information" },
  { key: "navMeditation", href: "/meditation" },
...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-1 left-0 right-0 z-[9999] flex justify-center px-2">
        <div className="w-full flex items-center justify-between px-5 py-3 bg-transparent backdrop-blur-xl">

          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 group">
            <Image src="/short logo.png" alt="MitrAI" width={40} height={25} />
            <span className="font-bold text-xl tracking-tight text-slate-800">
              Mitr<span className="text-indigo-600">AI</span>
            </span>
          </Link>
        ...
        <AnimatePresence>
          {isOpen && (
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <Link href="/login" className="text-center py-3 font-semibold text-slate-600">
                  {t("navLogin")}
                </Link>

                <Link
                  href="/signup"
                  className="text-center py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-200"
                >
                  {t("navJoinNow")}
                </Link>
              </div>
