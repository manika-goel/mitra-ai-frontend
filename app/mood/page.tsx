...
import {
  ArrowLeft, Activity, Sparkles, Zap, Heart, Info, Target, MoreHorizontal, Calendar as CalendarIcon
} from "lucide-react";
import { useRouter } from "next/navigation";
import { useLanguage } from "@/context/LanguageContext";

type Mood = "excited" | "sad" | "steady";

export default function MoodTracker() {
  const router = useRouter();
  const { t } = useLanguage();

  const [detectedMood, setDetectedMood] = useState<Mood>("steady");
  const [energyScore, setEnergyScore] = useState(0);
  const [moodHistory, setMoodHistory] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchMoodData = async () => {
      try {
        ...
        setDetectedMood(moodMap[data.avatar_state] || "steady");
        setEnergyScore((Number(data.latest_intensity) || 0) * 20);
  ...
  const avatarConfig = {
    excited: {
      color: "#6366F1",
      mouth: "M 80 85 Q 100 115 120 85",
      bounce: [-15, 0],
      headColor: "#A5B4FC",
      activity: t("activityHigh"),
    },
    sad: {
      color: "#94A3B8",
      mouth: "M 85 95 Q 100 85 115 95",
      bounce: [0, 5, 0],
      headColor: "#CBD5E1",
      activity: t("activityLow"),
    },
    steady: {
      color: "#6366F1",
      mouth: "M 85 85 Q 100 100 115 85",
      bounce: [0, -10, 0],
      headColor: "#A5B4FC",
      activity: t("activitySteady"),
    },
  };

  const config = avatarConfig[detectedMood];
      ...
      <div className="max-w-6xl w-full mx-auto flex flex-col h-full">
        ...
        <header className="flex items-center justify-between mb-6">
          <button
            ...
            className="p-3 bg-white rounded-2xl shadow-sm text-indigo-600 border border-white hover:scale-105 transition-transform"
          >
            <ArrowLeft size={20} />
          </button>

          <h1 className="text-2xl font-black text-slate-800 italic uppercase">
            {t("mood")} <span className="text-indigo-600">{t("sync")}</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0 mb-4">
          ...
          {/* Avatar */}
          <div className="lg:col-span-4 bg-white/80 backdrop-blur-xl p-8 rounded-[3rem] border border-white shadow-xl flex flex-col items-center">
            <div className="w-full flex justify-between items-center mb-4">
              <div className="flex gap-3 text-slate-300">
                <Zap size={18} className="hover:text-yellow-400 transition-colors" />
                <Heart size={18} className="hover:text-rose-400 transition-colors" />
                <Info size={18} className="hover:text-indigo-400 transition-colors" />
              </div>
              <span className="text-[9px] font-black uppercase bg-indigo-50 px-2 py-0.5 rounded text-indigo-500">
                {t("aiConnected")}
              </span>
            </div>

            <motion.div animate={{ y: config.bounce }} transition={{ duration: 0.8, repeat: Infinity }}>
              <svg width="130" height="180" viewBox="0 0 200 300">
                <rect x="70" y="110" width="60" height="110" rx="30" fill={config.color} />
                <circle cx="100" cy="70" r="40" fill={config.headColor} />
                <circle cx="85" cy="65" r="3" fill="white" />
                <circle cx="115" cy="65" r="3" fill="white" />
                <path d={config.mouth} stroke="white" strokeWidth="3" fill="none" />
              </svg>
            </motion.div>
            ...
            <div className="mt-4 p-4 bg-indigo-50/50 rounded-[2rem] border border-indigo-100 flex gap-3">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            {/* Graph */}
            <div className="bg-white/80 backdrop-blur-xl p-8 rounded-[3rem] border border-white shadow-sm flex-1 flex flex-col">
              <h3 className="text-xl font-black text-slate-800 flex items-center gap-2 italic mb-6">
                {t("emotionalFlow")} <Activity size={18} className="text-indigo-600" />
              </h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={moodHistory}>
                  ...
                  <YAxis hide />
                  <Tooltip />
                  <Area type="monotone" dataKey="score" stroke="#6366F1" fill="url(#colorScore)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="flex items-stretch gap-4 h-32">
              ...
              <div className="flex-1 bg-white p-6 rounded-[2.5rem] border border-white shadow-sm flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-black text-slate-800 italic">
                    {energyScore > 50 ? t("healthGood") : t("healthWait")}
                  </p>
                </div>
              </div>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => router.push("/calendar")}
                className="w-24 bg-white rounded-[2.5rem] border border-white shadow-xl flex flex-col items-center justify-center text-indigo-600"
              >
