    ...
    <>
      ...
      <main className="min-h-screen relative overflow-hidden px-6 py-20 
        ...
        >
          ...
          {/* Chat With Agent */}
          <motion.div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl shadow-lg p-10">
            <div className="flex items-center gap-3 mb-4">
              <MessageCircle className="text-indigo-500" />
              <h2 className="text-2xl font-semibold text-indigo-600">
                {t("supportChatTitle")}
            ...
            <button
              onClick={() => router.push("/settings/support/chat")}
              className="w-full py-4 bg-gradient-to-r from-indigo-500 to-blue-500 
              text-white rounded-full font-semibold shadow-md hover:scale-[1.03] transition"
            >
              {t("supportChatBtn")}
            </button>
          </motion.div>
          ...
          <motion.div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl shadow-lg p-10">
            <h2 className="text-2xl font-semibold text-indigo-600 mb-6">
              {t("supportRaiseConcern")}
            </h2>

            <div className="space-y-4">
              <select className="w-full p-4 rounded-xl border border-gray-300 text-gray-800">
                <option>{t("supportIssueApp")}</option>
                <option>{t("supportIssueAccount")}</option>
                <option>{t("supportIssueChat")}</option>
                <option>{t("supportIssuePrivacy")}</option>
                <option>{t("supportIssueOther")}</option>
              </select>
              ...
              <button
                onClick={() => setSubmitted(true)}
                className="w-full py-4 bg-indigo-500 text-white rounded-full font-semibold hover:bg-indigo-600 transition"
              >
          ...
          {/* Emergency Support */}
          <motion.div className="bg-yellow-50 border border-yellow-300 rounded-3xl p-10">
            <div className="flex items-center gap-3 mb-4">
              <AlertTriangle className="text-yellow-500" />
              <h2 className="text-xl font-semibold text-gray-800">
            ...
            <div className="space-y-3">
              <a href="tel:18005990019" className="flex items-center gap-3 p-4 bg-white rounded-xl shadow text-gray-800">
                <Phone className="text-yellow-500" />
                <span>{t("supportKiran")}</span>
              </a>

              <a href="tel:919820466726" className="flex items-center gap-3 p-4 bg-white rounded-xl shadow text-gray-800">
                <Phone className="text-yellow-500" />
                <span>{t("supportAasra")}</span>
              </a>
            </div>
          ...
          {/* FAQ */}
          <div className="bg-white/80 backdrop-blur-xl border border-white/50 rounded-3xl shadow-lg p-10">
            <h2 className="text-2xl font-semibold text-indigo-600 mb-6">
              {t("supportQuickHelp")}
            </h2>
            ...
            {faqs.map((item, i) => (
              <div key={i} className="border-b last:border-none">
                ...
                >
                  <span className="text-gray-900">{item.q}</span>
                  <ChevronDown className={`text-gray-900 transition ${openFaq === i ? "rotate-180" : ""}`} />
                </button>
