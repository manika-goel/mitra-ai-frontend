...
export default function MitrChatPro() {
  ...
  const handleSend = async (text = "") => {
    ...
    setMessages((prev) => [
      ...prev,
      { text: messageToSend, sender: "user", id: "u-" + Date.now() },
    ]);
    setInput("");
    setIsTyping(true);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { text: data.reply, sender: "bot", id: "b-" + Date.now() },
      ]);
    } catch (error) {
      setMessages((prev) => [
        ...prev,
        ...
        {isSidebarOpen && (
          ...
          >
            ...
            <div className="px-4 mb-4">
              <button
                onClick={() => setMessages([])}
                className="w-full flex items-center gap-3 p-3 bg-white border border-blue-100 rounded-2xl text-slate-600 hover:bg-blue-50 transition-all font-semibold shadow-sm"
              >
                <Plus size={18} /> {t("newConversation12")}
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 space-y-2">
              ...
              <div className="text-xs text-slate-400 ml-2 italic">
                {t("noRecentChats")}
              </div>
            </div>
            ...
            <div className="p-4 border-t border-blue-100 space-y-1">
              <Link href="/dashboard" className="flex items-center gap-3 p-3 rounded-xl hover:bg-white text-slate-600 text-sm font-medium">
                <LayoutDashboard size={18} /> {t("dashboard12")}
              </Link>
              <button className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white text-slate-600 text-sm font-medium">
                <Settings size={18} /> {t("settings12")}
              </button>
            </div>
          </motion.aside>
        )}
          ...
          <AnimatePresence mode="wait">
            {messages.length === 0 ? (
              <motion.div className="h-full flex flex-col items-center justify-center text-center">
                <motion.div className="mb-6 relative w-24 h-24 rounded-full overflow-hidden shadow-xl">
                  <Image src="/logo.png" alt="MitrAI" fill className="object-cover" />
                </motion.div>
                ...
                <div className="flex flex-wrap justify-center gap-3 max-w-md">
                  {[t("chipStress"), t("chipVent"), t("chipMotivation")].map((chip, index) => (
                    <button
                      key={index}
                      onClick={() => handleSend(chip)}
                      className="px-5 py-2.5 bg-white border border-indigo-50 text-indigo-600 text-sm font-semibold rounded-2xl hover:bg-indigo-50 transition-all shadow-sm"
                    >
                      {chip}
                    </button>
                  ))}
              ...
              messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[75%] p-4 rounded-3xl ${msg.sender === "user" ? "bg-indigo-600 text-white" : "bg-white border text-slate-700"}`}>
                    {msg.text}
                  </div>
                </div>
              ))
        ...
        <div className="p-6">
          <div className="max-w-4xl mx-auto flex items-center gap-2 bg-white p-2 rounded-[2rem] shadow-2xl border border-slate-100">
            <input
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder={t("chatPlaceholder12")}
              className="flex-1 p-3 ml-4 outline-none text-slate-700 placeholder:text-slate-300"
              onKeyDown={(e) => e.key === "Enter" && handleSend()}
            />
