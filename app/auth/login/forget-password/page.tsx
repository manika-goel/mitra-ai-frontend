  ...
  const handleReset = async (e: any) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(t("resetError") + (data.error || ""));
      }
    } catch (error) {
      console.error("Connection Error:", error);
      alert(t("forgotbackendError"));
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-sky-100 via-blue-100 to-indigo-100 px-6 py-16 text-center">
      ...
      >
        ...
        <input
          ...
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          className="w-full px-4 py-3 mb-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:outline-none text-gray-700"
        />
        ...
        <input
          ...
          value={newPassword}
          onChange={(e) => setNewPassword(e.target.value)}
          className="w-full px-4 py-3 mb-6 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-400 focus:outline-none text-gray-700"
        />
