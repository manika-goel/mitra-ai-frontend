  ...
  return (
    <main className="min-h-screen relative overflow-hidden flex flex-col items-center justify-center 
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8 }}
        className="relative z-10 max-w-md w-full bg-white/80 backdrop-blur-2xl border border-white/50 rounded-3xl shadow-2xl p-10"
      >
        ...
        <motion.button
          ...
          onClick={handleLogin}
          disabled={loading}
          className="w-full py-3 bg-indigo-500 hover:bg-indigo-600 text-white font-semibold rounded-full shadow-lg mb-4"
        >
        ...
        <Link href="/auth" className="w-full">
          <motion.button
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
            className="w-full py-3 border border-indigo-500 text-indigo-600 rounded-full font-semibold hover:bg-indigo-50"
          >
            ← {t("back")}
          </motion.button>
        </Link>
