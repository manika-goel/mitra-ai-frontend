  ...
  const handleReset = async () => {
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:5000/api/reset-password/anonymous",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (response.ok) {
        ...
        alert(`${t("error")} ${data.error || t("resetFailed")}`);
      }
    } catch (error) {
      alert(t("serverError"));
    } finally {
  ...
  return (
    <main className="min-h-screen relative overflow-hidden flex flex-col items-center justify-center 
      ...
      <motion.div
        ...
        className="absolute w-[380px] h-[380px] bg-blue-300/30 rounded-full blur-[100px] top-16 left-16"
      />
      ...
      >
        ...
        <input
          ...
          value={nickname}
          onChange={(e) => setNickname(e.target.value)}
          className="w-full px-6 py-4 rounded-full border mb-4 text-gray-800 placeholder:text-gray-500 focus:ring-2 focus:ring-indigo-500 outline-none"
        />

        {/* Security Question */}
        <div className="mb-4">
          <label className="block mb-2 text-sm text-gray-700 font-semibold px-2">
            {t("securityQuestionLabel")}
          </label>
          <div className="w-full px-6 py-4 rounded-2xl bg-indigo-50 border border-indigo-200 text-gray-800 text-sm font-semibold">
            {t("securityQuestion12")}
          </div>
        </div>
        ...
        <input
          type="password"
          placeholder={t("newPasswordPlaceholder12")}
          value={newPassword}
          onChange={(e) => setNewPassword(e.target.value)}
          className="w-full px-6 py-4 rounded-full border mb-8 text-gray-800 placeholder:text-gray-500 focus:ring-2 focus:ring-indigo-500 outline-none"
        />
        ...
        <motion.button
          ...
          onClick={handleReset}
          disabled={!answer || !nickname || !newPassword || loading}
          className="w-full py-5 bg-indigo-600 hover:bg-indigo-700 text-white text-lg font-semibold rounded-full shadow-lg disabled:opacity-40 transition-all"
        >
          {loading ? t("updating12") : t("resetAndLogin")}
        </motion.button>
      </motion.div>

      {/* Footer */}
      <p className="absolute bottom-6 text-gray-700 text-sm">
        {t("footer")}
      </p>
    </main>
  );
